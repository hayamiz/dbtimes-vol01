...
階層型モデルやネットワーク型モデルに基づくデータベースは，データのアクセスに一つ大きな問題を抱えていたのです。これらのデータベースにおいてデータの問い合わせを行う際には，データの構造がどのようになっているかを把握し，そしてどのような手順でデータを取得するかをプログラマ(データベースシステムのユーザ)が知っている必要がありました。例えば図\ref{214707_12Jul12}のデータベースにおいて社員Andrewのデータを取得するためには，「会社のデータを読みとり，部門Bのデータを読み取り，プロジェクトHelloのデータを読み取り，社員Andrewのデータを読み取る」という手順を指定してあげなければなりません。このように，欲しいデータにアクセスするために，プログラマがデータの案内(navigation)をしなければならないという意味で，階層型モデルやネットワーク型モデルは@<b>{ナビゲーショナルモデル}，またそれにもとづくデータベースシステムは@<b>{ナビゲーショナルデータベースシステム}と呼ばれます。
...
//footnote[rise-fn-4][DSL/ALPHAは関係演算(relational calculus)をベースとして，量化子$\exists, \forall$や，論理演算の記号$\lnot,\land,\lor$などがほぼそのまま用いられていました]
...
このクエリでは，EMPLOYEE(従業員)リレーションとDEPT(部門)リレーションが定義されているデータベースから，1階で働いていて(@<i>{D.FLOOR}# = 1)，年齢が40歳より上で(@<i>{E.AGE} $>$ 40)，マネージャーより給料が高い(@<i>{E.SALARY} $>$ @<i>{M.SALARY} @<b>{AND} @<i>{E.MANAGER} = @<i>{M.NAME} )従業員のデータを抜き出す，という問い合わせを行っています。
...
\begin{enumerate}
 \item @<b>{OPENR}(@<i>{descriptor}, @<i>{mode}, @<i>{relation_name})
 \item @<b>{GET}(@<i>{descriptor}, @<i>{tid}, @<i>{limit_tid}, @<i>{tuple}, @<i>{next_flag})
 \item @<b>{FIND}(@<i>{descriptor}, @<i>{key}, @<i>{tid}, @<i>{key_type})
 \item @<b>{PARAMD}(@<i>{descriptor}, @<i>{access_characteristics_structure})
 \item @<b>{PARAMI}(@<i>{index_descriptor}, @<i>{access_characteristics_structure})
 \item @<b>{INSERT}(@<i>{descriptor}, @<i>{tuple})
 \item @<b>{REPLACE}(@<i>{descriptor}, @<i>{tid}, @<i>{new_tuple})
 \item @<b>{DELETE}(@<i>{descriptor}, @<i>{tid})
 \item @<b>{CLOSER}(@<i>{descriptor})
\end{enumerate}
...
検索キーが与えられてから，タプルのデータが得られるまでの一連の流れを図\ref{190817_16Jul12}にまとめました。まずは検索キーが与えられると，@<b>{FIND}を呼び出してタプルIDを取得します。@<b>{FIND}の中では，ハッシュテーブルや木構造を用いて効率的にタプルIDの検索が行われます。そして，得られたタプルIDを引数として@<b>{GET}を呼び出すことで，ページを読み出し，タプルのデータが取得されます。
...
INGRESの開発において，何度もB$^+$-treeの導入が検討され，その度に採用が見送られてきたようです。しかしながら，1980年のINGRES開発を振り返る論文において，StonebrakerはB$^+$-treeの採用を見送ってきたのは @<raw>{|html|&ldquo;}major mistakes@<raw>{|html|&rdquo;}の一つであり，次にアクセスメソッドを再検討する際にはB$^+$-treeも実装するであろうと述べています。
...
//footnote[rise-fn-9][\verb|http://s2k-ftp.cs.berkeley.edu/ingres/|]
...
\begin{itemize}
 \item E. F. Codd, @<raw>{|html|&ldquo;}A Relational Model of Data for Large Shared Data
       Banks@<raw>{|html|&rdquo;}, @<i>{Communications of the ACM}, Volume 13, Number 6,
       1970
 \item E. F. Codd, @<raw>{|html|&ldquo;}A data base sublanguage founded on the relational
       calculus@<raw>{|html|&rdquo;}, @<i>{Proceedings of the 1971 ACM SIGFIDET workshop
       on Data Description, Access and Control}, 1971
 \item C. J. Date, E. F. Codd, @<raw>{|html|&ldquo;}The relational and network approaches: Comparison of the
       application programming interfaces@<raw>{|html|&rdquo;}, @<i>{Proceedings of the
       1974 ACM SIGFIDET workshop on Data Description, Access and
       Control}, 1975
 \item G. Held, M. Stonebraker, E. Wong, @<raw>{|html|&ldquo;}INGRES-A relational data
       base management system@<raw>{|html|&rdquo;}, @<i>{Proceedings of AFIPS 1975 NCC},
       Volume 44, 1975
 \item M. Stonebraker, G. Held, E. Wong, P. Kreps, @<raw>{|html|&ldquo;}The design and
       implementation of INGRES@<raw>{|html|&rdquo;}, @<i>{ACM Transactions on Database
       Systems}, Volume 1, Issue 3, 1976
 \item G. Held, M. Stonebraker, @<raw>{|html|&ldquo;}B-trees re-examined@<raw>{|html|&rdquo;}, @<i>{Communications of the
       ACM}, Volume 21, Issue 2, 1978
 \item M. Stonebraker, @<raw>{|html|&ldquo;}Retrospection on a database system@<raw>{|html|&rdquo;}, @<i>{ACM
 ...
 \item D. Chamberlin, M. Astrahan, M. Blasgen, J. Gray, W. F. King,
       ...
       M. Schkolnick, D. Slutz, I. Traiger, B. Wade, R. Yost, @<raw>{|html|&ldquo;}A
       history and evaluation of System R@<raw>{|html|&rdquo;}, @<i>{Communications of the
       ACM}, Volume 24, Issue 10, 1981
 \item M. Stonebraker, J. Woodfill, J. Ranstrom, M. Murphy, M. Meyer,
       E. Allman, @<raw>{|html|&ldquo;}Performance enhancements to a relational database
       system@<raw>{|html|&rdquo;}, @<i>{ACM Transactions on Database Systems}, Volume 8,
       Issue 2, 1983
 \item Relational Technology Inc., @<raw>{|html|&ldquo;}INGRES The Distributed SQL
       Relational Database System Press Kit@<raw>{|html|&rdquo;}, 1987
 \item National Research Council, @<raw>{|html|&ldquo;}Funding a Revolution: Government
       Support for Computing Research@<raw>{|html|&rdquo;}, Natl Academy Pr, 1999
 \item M. Stonebraker, J. M. Hellerstein, @<raw>{|html|&ldquo;}What Goes Around Comes
       Around@<raw>{|html|&rdquo;}, @<i>{Readings in Database Systems, Fourth Edition},
       Morgan Kaufmann, 2005
 \item A. Ances, @<raw>{|html|&ldquo;}RDBMS Workshop: Ingres and Sybase@<raw>{|html|&rdquo;}, Computer
       History Museum, 2007
\end{itemize}
