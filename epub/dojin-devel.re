...
本誌の組版には TeX Live 2011 を用いている。文書のクラスには奥村先生の@<tt>{jsbook.cls} を利用しており，@<tt>{tombow} オプションを利用することでトンボ付きの出力を得ている。@<tt>{tombow}オプションを利用する際には，併せて@<tt>{papersize}オプションを利用しないと，文書のサイズとしてはトンボ無しで出力されてしまうので注意が必要である。
