...
//raw[|html|<div style="text-align: right">suu-g (@suu_g)</div>]

ヰンターネットが始まってから20年，ARPANETから数えると40年以上。
...
今年は，どんな問題が起きたのでしょうか？振り返ってみましょう。
  ...
  * （日本）児童ポルノに関するDNSブロッキング立法・ISPにおける運用が開始
  * SOPAにGo Daddyが賛成を表明するも1日で撤回
  * SOPAに反対を表明するためWikipediaが24時間閉鎖する
  * SOPAに反対を表明してAnonymousがOperation Blackoutを表明
  * v6-v4フォールバック問題の対策法としてAAAAフィルタリングが検討される
  * Google, AAAAフィルタを導入している可能性のあるDNSサーバに対してAAAAレコードを回答しない運用を宣言
  * Ghost Domain問題の発表
  * ニフティクラウドのDNS障害問題
  * GMOクラウドのDNSで謎の障害がある
  * さくらのDNSに（サブ）ドメインジャッキングが可能な脆弱性が発覚
  * JPRSがあらためてコンテンツサーバとキャッシュサーバの兼用の危険性を告知
  * BIND, 長さ0のRDATAにより異常終了する脆弱性（重複をお許しください）
  * お名前.com，忍者ツールズのNSレコードを規約に基づき変更
  * 今年の夏のDNS祭りはBINDではなくNSD
...

{
  ...
  Query: [{
    Name: @<raw>{|html|&ldquo;}www.example.jp.@<raw>{|html|&rdquo;},
    Type: @<raw>{|html|&ldquo;}AAAA@<raw>{|html|&rdquo;},
    Class: @<raw>{|html|&ldquo;}IN@<raw>{|html|&rdquo;},
  }],
  Answer: [],
  Authoritative: [],
  Additional: []
...

{
  ...
  Answer: [{
    ...
    Class: @<raw>{|html|&ldquo;}IN@<raw>{|html|&rdquo;},
    TTL: 86400,
    RData: @<raw>{|html|&ldquo;}2001:DB8::163@<raw>{|html|&rdquo;}
  }],
  Authoritative: [],
  Additional: []
}
...

{
  ...
  Answer: [],
  Authoritative: [],
    Name: @<raw>{|html|&ldquo;}example.jp.@<raw>{|html|&rdquo;},
    Type: @<raw>{|html|&ldquo;}NS@<raw>{|html|&rdquo;},
    Class: @<raw>{|html|&ldquo;}IN@<raw>{|html|&rdquo;},
    TTL: 86400,
    RData: @<raw>{|html|&ldquo;}ns.example.jp@<raw>{|html|&rdquo;}
  Additional: []
    Name: @<raw>{|html|&ldquo;}ns.example.jp@<raw>{|html|&rdquo;},
    Type: @<raw>{|html|&ldquo;}AAAA@<raw>{|html|&rdquo;},
    Class: @<raw>{|html|&ldquo;}IN@<raw>{|html|&rdquo;},
    TTL: 86400,
    RData: @<raw>{|html|&ldquo;}2001:DB8::160@<raw>{|html|&rdquo;}
}
//}
...
これをJSON風に書くと，@<list>{jsondns}のようになってます。
（正確には，DNSSECやEDNS0対応等でフラグはもう幾つかある）
読者の皆様におかれましてはこのパケットの意味を判断しようとしてくれると
期待しますが，勘の良い人だと，このプロトコルのもつ怪しさをなんとなく
感じ取ったかも知れません。
//footnote[dns-fn-1][http://www.ietf.org/rfc/rfc1034.txt Section 3-7]
//footnote[dns-fn-2][http://www.ietf.org/rfc/rfc1034.txt Section 4-1]
...
//footnote[dns-fn-3][http://www.ietf.org/rfc/rfc2181.txt]
...
基本ったら最強ね。
//footnote[dns-fn-4][http://www.e-ontap.com/summary/]
...
これが @<raw>{|html|&ldquo;}Ghost Domain@<raw>{|html|&rdquo;} 
@<fn>{dns-fn-5}
問題（日本語で「幽霊ドメイン」問題）です。 
その原因は，複数のサーバソフトウェアに同様に存在した実装不具合でした。
//footnote[dns-fn-5][www.isc.org/files/imce/ghostdomain_camera.pdf]
...
@<fn>{dns-fn-7} に詳しい
のでそちらをご覧ください。
//footnote[dns-fn-6][http://dnsops.jp/bof/20120425/20120425-DNSOPS.jp-BoF_Ghost_Domain_Names_v02.pdf]
//footnote[dns-fn-7][http://www.geekpage.jp/blog/?id=2012/3/21/1]
...
//footnote[dns-fn-8][さくらはたまたま注目を浴びただけで，問題を抱えたサービスプロバイダは他にもたくさんある…が，その話はまた今度]
